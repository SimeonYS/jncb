...
class JjncbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('jncb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `jncb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM jncb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO jncb (date, title, link, content)"
